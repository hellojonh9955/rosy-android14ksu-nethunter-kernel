...
struct nethunter_hidg_descr {
	unsigned short report_desc_length;
	char report_desc[63];
};

static struct nethunter_hidg_descr nethunter_hid_descriptors = {
		...
		const struct usb_ctrlrequest *ctrl)
{
	...
	case ((USB_DIR_IN | USB_TYPE_STANDARD | USB_RECIP_INTERFACE) << 8
		...
		case HID_DT_REPORT:
			...
			length = min_t(unsigned short, length, nethunter_hid_descriptors.report_desc_length);
			memcpy(req->buf, nethunter_hid_descriptors.report_desc, length);
			goto respond;
			break;
